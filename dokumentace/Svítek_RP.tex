...
Ovladač je vybaven dvěma dvouosými joystiky, ketré jsou 
...
že baterie dokáže trvale dodávat proud až $40 \times 1{,}3 = 52$~A,
...
Použití typu \texttt{int16\_t} (16bitové celé číslo se znaménkem)
zajišťuje přesnost $\pm$32\,767 kroků a~konstantní velikost 2~bajty
na kanál nezávisle na platformě.

%%-------------------------------------------------------------------------
\section{Firmware vysílače (ovladač)}

\subsection{Struktura kódu}

Firmware ovladače provádí v~hlavní smyčce tři operace: přečte aktuální
hodnoty z~analogových vstupů, uloží je do struktury \texttt{ControlPacket}
...
void loop() {
    packet.throttle = map(analogRead(PIN_THROTTLE), 0, 4095, 0, 1000);
    packet.pitch    = map(analogRead(PIN_PITCH),    0, 4095, -500, 500);
    packet.roll     = map(analogRead(PIN_ROLL),     0, 4095, -500, 500);
    packet.yaw      = map(analogRead(PIN_YAW),      0, 4095, -500, 500);
...
%%-------------------------------------------------------------------------
\section{Firmware přijímače (letadlo)}
...
void loop() {
    if (HC12.available() >= (int)sizeof(packet)) {
        HC12.readBytes((uint8_t*)&packet, sizeof(packet));

        setThrottle(packet.throttle);
        setElevator(packet.pitch);
        setAilerons(packet.roll);
    }
}
\end{lstlisting}
